...
{
    // start datatables
    var $column_order = array(null, 'barcode', 'p_item.name', 'kategori_name', 'unit_name', 'colour_name', 'price', 'stock'); //set column field database for datatable orderable
    ...
    {
        $this->db->select('p_item.*, p_kategori.name as kategori_name, p_unit.name as unit_name, p_colour.name as colour_name');
        $this->db->from('p_item');
        $this->db->join('p_kategori', 'p_item.kategori_id = p_kategori.kategori_id');
        $this->db->join('p_unit', 'p_item.unit_id = p_unit.unit_id');
        $this->db->join('p_colour', 'p_item.colour_id = p_colour.colour_id', 'left'); // Tambahkan join tabel warna
        $i = 0;
    ...
    public function add($post)
    {
        $params = [
            ...
            'unit_id' => $post['unit'],
            'colour_id' => $post['colour'], // Menambahkan warna saat menambahkan item
            'price' => $post['price'],
            'image' => $post['image'],
        ];
    ...
    public function edit($post)
    {
        $params = [
            ...
            'kategori_id' => $post['kategori'],
            'unit_id' => $post['unit'],
            'colour_id' => $post['colour'], // Menambahkan warna saat mengedit item
            'price' => $post['price'],
    ...
    {
        $this->db->select('barcode, name, kategori_id, unit_name, price, stock, item_id, kategori_name');
        $this->db->from('p_item');
        $this->db->join('p_kategori', 'p_kategori.kategori_id = p_item.kategori_id');
        $query = $this->db->get();
        return $query->result();
